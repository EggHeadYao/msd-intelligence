...
Formal commands default to canonical paths from `InferenceArtifactPaths` but also accept explicit paths. Smoke and diagnostic outputs should be written outside the formal artifact directory.

Spark commands on this host must disable event logging unless a writable event log directory is configured. See the repository `AGENTS.md` for the local Spark invocation constraints.
