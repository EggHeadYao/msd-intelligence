...
public final class ParquetBfsOutputVerifier {
  private ParquetBfsOutputVerifier() {}

  public static void main(String[] args) throws Exception {
    if (args.length != 3) {
      System.err.println("Usage: ParquetBfsOutputVerifier <adjacency.parquet> <final-dir> <source_id>");
      System.exit(1);
    }
    ReferenceBfs.Result expected =
        ReferenceBfs.compute(ParquetRecords.readAdjacency(Path.of(args[0])), args[2]);
  ...
  private static Map<String, ArtistDistance> readActual(Path path) throws Exception {
    ...
    for (ArtistDistance row : ParquetRecords.readArtistDistances(path)) {
