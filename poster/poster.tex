...
\newcommand{\eramark}{%
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[anchor=north,text=postermuted,
      font=\fontsize{15}{17}\selectfont\bfseries] at (23.0,.08) {1920s};
    \node[anchor=north,text=postermuted,
      font=\fontsize{15}{17}\selectfont\bfseries] at (35.0,.08) {1960s};
    \node[anchor=north,text=postermuted,
      font=\fontsize{15}{17}\selectfont\bfseries] at (47.0,.08) {2000s};
  ...
  \begin{targetcard}[height=22.3cm]
    ...
    \begin{center}
      ...
      \begin{tikzpicture}[
        ...
        outcome/.style={circle,minimum size=3.35cm,line width=1.05mm,
          fill=white,fill opacity=.50,text opacity=1,align=center,
          font=\fontsize{24}{27}\selectfont\bfseries}
      ]
        \draw[posterteal!70!posterink,line width=.72mm] (12.35,6.60) circle (1.35cm);
        \draw[posterteal!55!posterink,line width=.48mm] (12.35,6.60) circle (.90cm);
        \fill[postergold!88!black] (12.35,6.60) circle (.38cm);
        \fill[white] (12.35,6.60) circle (.10cm);
        \node[font=\fontsize{20}{23}\selectfont\bfseries,text=posterteal!92!posterink]
          at (12.35,4.72) {QUERY SONG};
        ...
        \draw[flow,draw=posterteal!80!posterink]
          (11.55,5.50) -| (catalog.north);
        \draw[flow,draw=posterpurple!86!posterink]
          (12.35,5.20) -- (discover.north);
        \draw[flow,draw=postergold!90!black]
          (13.15,5.50) -| (remember.north);
      \end{tikzpicture}%
      }
    \end{center}
    \vfill
    \begin{center}
      {\fontsize{21}{25}\selectfont\bfseries\color{postermuted}ONE REUSABLE DATA FOUNDATION\par}
      \vspace{.18cm}
      \resizebox{.92\linewidth}{!}{%
      \begin{tikzpicture}[
        x=1cm,y=1cm,>=Latex,
        data/.style={align=center,font=\fontsize{22}{25}\selectfont\bfseries},
...
\begin{textblock}{29.5}(35.25,24.8)
  \begin{discoverycard}[height=22.3cm]
    ...
    \begin{center}
      ...
      \begin{tikzpicture}[
        x=1cm,y=1cm,>=Latex,
        source/.style={circle,minimum size=2.48cm,line width=.92mm,
          draw=posterpurple!80!posterink,fill=white,fill opacity=.44,text opacity=1,
          align=center,font=\fontsize{20}{23}\selectfont\bfseries},
        flow/.style={-{Latex[length=3.4mm,width=2.5mm]},draw=posterpurple!76!posterink,
          line width=.92mm}
      ]
        \node[align=center,font=\fontsize{26}{30}\selectfont\bfseries,text=posterink]
          (catalog) at (1.55,4.40) {1M\\TRACKS};

        \node[source] (audio) at (7.2,8.55) {AUDIO};
        \node[source] (graph) at (7.2,5.78) {GRAPH};
        \node[source] (bfs) at (7.2,3.02) {ARTIST\\DISTANCE};
        \node[source] (tags) at (7.2,.25) {TAGS};

        \node[align=center,font=\fontsize{26}{30}\selectfont\bfseries,
          text=posterpurple!94!posterink] (shortlist) at (15.9,4.40)
          {$\leq$1,000\\CANDIDATES};
        \node[circle,minimum size=3.25cm,line width=1.2mm,
          draw=posterpurple!90!posterink,fill=posterpurple!13,
          align=center,font=\fontsize{28}{31}\selectfont\bfseries,text=posterink]
          (topk) at (22.35,4.40) {TOP\\20};
    ...
    \begin{center}
      ...
      \begin{tikzpicture}[x=1cm,y=1cm]
        ...
        \node[align=center,font=\fontsize{33}{37}\selectfont\bfseries,
      ...
      \vspace{.58cm}
      {\fontsize{23}{27}\selectfont\bfseries\color{postermuted}
...
\begin{textblock}{29.5}(66.5,24.8)
  \begin{graphcard}[height=22.3cm]
    ...
    \vspace{-.48cm}
    \begin{center}
      {\fontsize{65}{68}\selectfont\bfseries\color{posterpurple!95!posterink}14.4$\times$\par}
      \vspace{-.08cm}
      {\fontsize{30}{34}\selectfont\bfseries Spark acceleration\par}
    \end{center}
    \vspace{.28cm}
    \begin{center}
      \resizebox{.88\linewidth}{!}{%
      \begin{tikzpicture}[x=1cm,y=1cm,font=\fontsize{21}{24}\selectfont]
        ...
        \node[anchor=north,text=postermuted] at (14.35,-.12) {median wall time \textperiodcentered\ lower is better};
      \end{tikzpicture}%
      }
    \end{center}
    \vfill
    \begin{center}
      {\fontsize{25}{29}\selectfont\bfseries\color{posterink}
        5 RUNS $\times$ 4 COMBINATIONS\par}
      \vspace{.12cm}
      {\fontsize{22}{26}\selectfont\color{postermuted}
...
\begin{textblock}{29.5}(4,66.2)
  \begin{investmentcard}[height=17cm]
    ...
    \begin{center}
      \resizebox{.94\linewidth}{!}{%
      \begin{tikzpicture}[x=1cm,y=1cm]
        \node[anchor=east,font=\fontsize{47}{51}\selectfont\bfseries,
          text=postergold!94!black] at (5.0,6.45) {1M};
        \node[anchor=west,align=left,font=\fontsize{25}{29}\selectfont\bfseries,
    ...
    \vfill
    \begin{center}
      {\fontsize{22}{26}\selectfont\bfseries\color{postermuted}
        HADOOP \textbullet\ PARQUET \textbullet\ DRILL \textbullet\ SPARK \textbullet\ FAISS\par}
    \end{center}
  \end{investmentcard}
\end{textblock}
...
\begin{textblock}{60.75}(35.25,66.2)
  \begin{eracard}[height=17cm]
    ...
    \begin{center}
      ...
      \begin{tikzpicture}[x=1cm,y=1cm]
        \node[anchor=west,font=\fontsize{68}{71}\selectfont\bfseries,
          text=posterpurple!95!posterink] at (0,6.45) {7.24};
        \node[anchor=west,font=\fontsize{27}{31}\selectfont\bfseries,
          text=posterink] at (.25,4.85) {TEST RMSE};
        \node[anchor=west,font=\fontsize{43}{47}\selectfont\bfseries,
          text=posterink] at (.15,2.85) {4.82};
        \node[anchor=west,font=\fontsize{25}{29}\selectfont\bfseries,
          text=postermuted] at (.25,1.65) {TEST MAE};
        \node[anchor=west,font=\fontsize{26}{30}\selectfont\bfseries,
          text=posterpurple!92!posterink] at (.20,.35) {23\% LOWER RMSE};

        \draw[posterpurple!32,line width=.60mm] (12.6,-.15) -- (12.6,7.75);

        \node[anchor=east,font=\fontsize{22}{25}\selectfont\bfseries] at (20.4,6.75) {Ridge};
        \fill[posterpurple!24,rounded corners=.6mm] (21.0,6.35) rectangle (48.0,7.15);
        \node[anchor=west,font=\fontsize{22}{25}\selectfont\bfseries] at (48.45,6.75) {9.41};

        \node[anchor=east,font=\fontsize{22}{25}\selectfont\bfseries] at (20.4,5.20) {Audio};
        \fill[posterpurple!40,rounded corners=.6mm] (21.0,4.80) rectangle (44.98,5.60);
        \node[anchor=west,font=\fontsize{22}{25}\selectfont\bfseries] at (45.43,5.20) {8.36};

        \node[anchor=east,font=\fontsize{22}{25}\selectfont\bfseries] at (20.4,3.65) {Metadata};
        \fill[posterpurple!55,rounded corners=.6mm] (21.0,3.25) rectangle (43.15,4.05);
        \node[anchor=west,font=\fontsize{22}{25}\selectfont\bfseries] at (43.60,3.65) {7.72};

        \node[anchor=east,font=\fontsize{22}{25}\selectfont\bfseries] at (20.4,2.10) {Fused};
        \fill[posterpurple!73,rounded corners=.6mm] (21.0,1.70) rectangle (42.04,2.50);
        \node[anchor=west,font=\fontsize{22}{25}\selectfont\bfseries] at (42.49,2.10) {7.33};

        \node[anchor=east,font=\fontsize{22}{25}\selectfont\bfseries,
          text=posterpurple!95!posterink] at (20.4,.55) {Ensemble};
        \fill[posterpurple!95!posterink,rounded corners=.6mm] (21.0,.15) rectangle (41.78,.95);
        \node[anchor=west,font=\fontsize{22}{25}\selectfont\bfseries,
          text=posterpurple!95!posterink] at (42.23,.55) {7.24};
      \end{tikzpicture}%
      }
      \vspace{.58cm}
      {\fontsize{22}{26}\selectfont\bfseries\color{postermuted}
