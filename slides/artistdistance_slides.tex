...
\begin{frame}{Spark: keep the graph, move the frontier}

\centering
\begin{tikzpicture}[
	>=stealth,
	stage/.style={rounded corners=2pt,minimum width=1.95cm,minimum height=2.15cm,
		align=center,draw=structbgcolor,line width=1.05pt,fill=structbgcolor!12,
		font=\small},
	flow/.style={->,line width=1.1pt,draw=black!48}
]
	\node[rounded corners=2pt,draw=black!25,fill=black!3,minimum width=8.8cm,
		minimum height=.58cm,font=\small\bfseries] at (0,2.5)
		{Same level: visited $=\{A,B,C\}$, frontier $=\{B,C\}$};

	\node[stage] (state) at (-4.6,.55) {\textbf{Distributed state}\\[5pt]
		Persisted adjacency\\$+$\\frontier $\{B,C\}$};
	\node[stage] (expand) at (-2.3,.55) {\textbf{Join + explode}\\[5pt]
		$A,D,E$\\$B,E,F$\\[3pt]
		{\scriptsize candidate rows}};
	\node[stage] (filter) at (0,.55) {\textbf{Left-anti join}\\[5pt]
		remove visited\\$A,B$\\[3pt]
		$D,E,E,F$};
	\node[stage] (group) at (2.3,.55) {\textbf{Group by artist}\\[5pt]
		$D\leftarrow B$\\
		$E\leftarrow B$\\
		$F\leftarrow C$};
	\node[stage] (next) at (4.6,.55) {\textbf{Checkpoint}\\[5pt]
		next frontier\\$\{D,E,F\}$\\[3pt]
		{\scriptsize extend visited}};
	\draw[flow] (state) -- (expand);
	\draw[flow] (expand) -- (filter);
	\draw[flow] (filter) -- (group);
	\draw[flow] (group) -- (next);

	\node[rounded corners=2pt,draw=structbgcolor,fill=structbgcolor!14,
		minimum width=10.6cm,minimum height=1.05cm,align=center,font=\small]
		at (0,-1.65) {\textbf{Adjacency remains in \texttt{MEMORY\_AND\_DISK}}\\
		Only the distributed frontier and visited set evolve; local checkpoints truncate lineage.};
	\node[font=\scriptsize\bfseries,text=black!60] at (0,-2.48)
		{All levels execute inside one Spark application.};
\end{tikzpicture}
