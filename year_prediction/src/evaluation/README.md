...
- `validate_model.py`: reloads a trained custom Ridge model, recomputes train and validation results, and verifies every saved validation prediction.

`validate_model.py` validates artifact consistency. It does not retrain the model, select hyperparameters, or evaluate the held-out test split.
