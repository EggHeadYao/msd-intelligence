...
def _make_feature_columns() -> list[str]:
    ...
    for event in ("beat", "bar", "tatum"):
        columns.extend(
            f"{event}_interval_{stat}" for stat in ("median", "iqr", "cv")
        )
    columns.extend(
        (
...
if len(FEATURE_COLUMNS) != FEATURE_COUNT:
    raise RuntimeError(f"Expected {FEATURE_COUNT} audio features, got {len(FEATURE_COLUMNS)}")
...
def _weighted_summary(values: np.ndarray, weights: np.ndarray) -> list[float]:
    ...
    stds = np.zeros(matrix.shape[1], dtype=np.float64)
    q10 = np.zeros(matrix.shape[1], dtype=np.float64)
    q90 = np.zeros(matrix.shape[1], dtype=np.float64)
...
def _pool_interval(
    ...
    overlap = np.maximum(
        0.0,
        np.minimum(segment_ends, interval_end) - np.maximum(segment_starts, interval_start),
    )
...
def aggregate_audio_features(
    ...
    """Aggregate one track's variable-length arrays into 308 finite features."""
    ...
    peak_ratio = np.divide(
        loudness_max_time,
        segment_lengths,
        out=np.zeros_like(loudness_max_time),
        where=segment_valid,
    )
    ...
    loudness_summary = np.asarray(_weighted_summary(loudness_matrix, weights)).reshape(4, 4)
    ...
    features.extend(
        (
            float(valid_segment_count),
            valid_segment_count / duration if duration > 0 else 0.0,
        ),
    )
    ...
    for left, right, valid in zip(pitches[:-1], pitches[1:], pair_valid, strict=True):
        ...
        if valid and denominator > EPSILON and np.all(np.isfinite(left)) and np.all(np.isfinite(right)):
    ...
    for stat in (np.mean, np.std):
        values = (
            stat(valid_timbre_deltas, axis=0)
            if valid_timbre_deltas.size
            else np.zeros(12)
        )
        features.extend(np.nan_to_num(values).tolist())
    ...
    for event in ("beats", "bars", "tatums", "sections"):
        ...
        features.extend((float(count), count / duration if duration > 0 else 0.0))
    ...
    local_bpm_median = float(np.median(local_bpm)) if local_bpm.size else 0.0
    local_bpm_mean = float(np.mean(local_bpm)) if local_bpm.size else 0.0
    local_bpm_cv = (
    ...
    beat_median = float(np.median(beat_intervals)) if beat_intervals.size else 0.0
    bar_intervals = interval_values["bars"]
    tatum_intervals = interval_values["tatums"]
    bar_median = float(np.median(bar_intervals)) if bar_intervals.size else 0.0
    tatum_median = float(np.median(tatum_intervals)) if tatum_intervals.size else 0.0
    features.extend(
        (
            local_bpm_median,
            local_bpm_cv,
            tempo_deviation,
            bar_median / beat_median if beat_median > EPSILON else 0.0,
            beat_median / tatum_median if tatum_median > EPSILON else 0.0,
        ),
    )
    ...
    section_ends, section_lengths, section_valid = _event_intervals(section_starts, duration)
    ...
    features.append(
        ...
        if valid_section_lengths.size and duration > 0
        else 0.0,
    )
    ...
    for left, right in zip(section_pools[:-1], section_pools[1:], strict=True):
        ...
        section_timbre_changes.append(float(np.mean(np.abs(right[12:24] - left[12:24]))))
    ...
    for values in (
        ...
        features.extend(
            (
                float(np.mean(array)) if array.size else 0.0,
                float(np.std(array)) if array.size else 0.0,
            ),
        )
    ...
    features.extend(
        (
            1.0 - valid_segment_count / segment_count if segment_count else 0.0,
            float(np.sum(weights) / duration) if duration > 0 else 0.0,
        ),
    )
    ...
    if not np.all(np.isfinite(result)):
        raise ValueError("Aggregated features contain NaN or Inf")
    return result
