...
def merge_candidates(groups: Sequence[Sequence[Candidate]], query_track_id: str) -> list[Candidate]:
    ...
    merged: dict[str, Candidate | dict[str, object]] = {}
    for group in groups:
        for candidate in group:
            if candidate.track_id == query_track_id:
                continue
            state = merged.get(candidate.track_id)
            if state is None:
                merged[candidate.track_id] = candidate
                continue
            if isinstance(state, Candidate):
                state = {
                    "sources": set(state.sources),
                    "scores": dict(state.recall_scores),
                    "ranks": dict(state.source_ranks),
                }
                merged[candidate.track_id] = state
    ...
    return [
        state if isinstance(state, Candidate) else Candidate(
            track_id=track_id,
            sources=frozenset(state["sources"]),
            recall_scores=state["scores"],
            source_ranks=state["ranks"],
        )
